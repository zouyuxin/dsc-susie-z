...
dscout = dscout[,-c(2,6,10,12:16,19)]
colnames(dscout) = c('DSC', 'pve', 'n_signal', 'meta','N_in','method','ld_method','estimate_residual_variance','converged','susie.pip', 'add_z')
# only success cases
...
```{r}
dscout.susieb.est = dscout %>% filter(method == 'susie_bhat') %>% filter(estimate_residual_variance==TRUE)
dscout.susieb.notest = dscout %>% filter(method == 'susie_bhat') %>% filter(estimate_residual_variance==FALSE)

dscout.susieb.addz.est = dscout %>% filter(method == 'susie_bhat_add_z') %>% filter(estimate_residual_variance==TRUE)
dscout.susieb.addz.notest = dscout %>% filter(method == 'susie_bhat_add_z') %>% filter(estimate_residual_variance==FALSE)
...
dscout.susieb.list = list('in_sample.est' = dscout.susieb.est %>% filter(ld_method == 'in_sample'),
                          ...
                          'out_sample.addz.est' = dscout.susieb.addz.est,
                          'out_sample.addz.notest' = dscout.susieb.addz.notest)
dat = list()
...
for(Rtype in names(dat)){
  for(j in 1:nrow(dscout.susieb.list[[Rtype]])){
    datj = cbind(dscout.susieb.list[[Rtype]]$susie.pip[[j]], as.integer(dscout.susieb.list[[Rtype]]$meta[[j]]$true_coef!=0))
...
pip_cali = list('in_sample.est'=matrix(NA, nrow(bins), 3),'in_sample.notest'=matrix(NA, nrow(bins), 3), 'out_sample.est'=matrix(NA, nrow(bins), 3),'out_sample.notest'=matrix(NA, nrow(bins), 3), 'out_sample.addz.est'=matrix(NA, nrow(bins), 3), 'out_sample.addz.notest'=matrix(NA, nrow(bins), 3))
...
saveRDS(roc, 'output/r_compare_add_z_lambda_SuSiEb_ROC.rds')
```

```{r}
roc = readRDS('output/r_compare_add_z_lambda_SuSiEb_ROC.rds')
chunks = 0
smooth = FALSE
