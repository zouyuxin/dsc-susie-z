...
dscout.equal.finemap.in_sample = dscout.equal.finemap[dscout.equal.finemap$ld_method == 'in_sample',]
dscout.equal.finemap.out_sample = dscout.equal.finemap[dscout.equal.finemap$ld_method == 'out_sample',]
```

```{r eval=FALSE}
dat = list('in_sample'=matrix(NA, 0, 2), 'out_sample'=matrix(NA, 0, 2))
for(j in 1:nrow(dscout.equal.finemap.in_sample)){
  datj.in_sample = cbind(dscout.equal.finemap.in_sample$pip[[j]], dscout.equal.finemap.in_sample$meta[[j]]$true_coef)
  dat$in_sample = rbind(dat$in_sample, datj.in_sample)
  datj.out_sample = cbind(dscout.equal.finemap.out_sample$pip[[j]], dscout.equal.finemap.out_sample$meta[[j]]$true_coef)
  dat$out_sample = rbind(dat$out_sample, datj.out_sample)
}
colnames(dat$in_sample) = c('pip', 'truth')
colnames(dat$out_sample) = c('pip', 'truth')
