...
dscout = dscout[,-c(2,6,12:16,19)]
colnames(dscout) = c('DSC', 'pve', 'n_signal', 'meta','N_in','method','ld_method','lambda','estimate_residual_variance','converged','susie.pip', 'add_z')
# only success cases
dscout = dscout[!is.na(dscout$converged), ]
dscout = dscout[dscout$converged == 1,]
```

## SuSiE RSS

```{r}
dscout.susierss.est = dscout %>% filter(method == 'susie_rss') %>% filter(estimate_residual_variance==TRUE) %>% filter(lambda == 0)
dscout.susierss.est.lambda = dscout %>% filter(method == 'susie_rss') %>% filter(estimate_residual_variance==TRUE) %>% filter(lambda == 1e-6)

dscout.susierss.notest = dscout %>% filter(method == 'susie_rss') %>% filter(estimate_residual_variance==FALSE) %>% filter(lambda == 0)
dscout.susierss.notest.lambda = dscout %>% filter(method == 'susie_rss') %>% filter(estimate_residual_variance==FALSE) %>% filter(lambda == 1e-6)

dscout.susierss.addz.est = dscout %>% filter(method == 'susie_rss_add_z') %>% filter(estimate_residual_variance==TRUE) %>% filter(lambda == 0)
dscout.susierss.addz.est.lambda = dscout %>% filter(method == 'susie_rss_add_z') %>% filter(estimate_residual_variance==TRUE) %>% filter(lambda == 1e-6)

dscout.susierss.addz.notest = dscout %>% filter(method == 'susie_rss_add_z') %>% filter(estimate_residual_variance==FALSE) %>% filter(lambda == 0)
dscout.susierss.addz.notest.lambda = dscout %>% filter(method == 'susie_rss_add_z') %>% filter(estimate_residual_variance==FALSE) %>% filter(lambda == 1e-6)

dscout.susierss.list = list('in_sample.est' = dscout.susierss.est %>% filter(ld_method == 'in_sample'),
                            'in_sample.est.lambda' = dscout.susierss.est.lambda %>% filter(ld_method == 'in_sample'),
                            'in_sample.notest' = dscout.susierss.notest %>% filter(ld_method == 'in_sample'),
                            'in_sample.notest.lambda' = dscout.susierss.notest.lambda %>% filter(ld_method == 'in_sample'),
                            'out_sample.est' = dscout.susierss.est %>% filter(ld_method == 'out_sample'),
                            'out_sample.est.lambda' = dscout.susierss.est.lambda %>% filter(ld_method == 'out_sample'),
                            'out_sample.notest' = dscout.susierss.notest %>% filter(ld_method == 'out_sample'),
                            'out_sample.notest.lambda' = dscout.susierss.notest.lambda %>% filter(ld_method == 'out_sample'),
                            'out_sample.addz.est' = dscout.susierss.addz.est,
                            'out_sample.addz.est.lambda' = dscout.susierss.addz.est.lambda,
                            'out_sample.addz.notest' = dscout.susierss.addz.notest,
                            'out_sample.addz.notest.lambda' = dscout.susierss.addz.notest.lambda)
...
for(Rtype in names(dat)){
  for(j in 1:nrow(dscout.susierss.list[[Rtype]])){
    datj = cbind(dscout.susierss.list[[Rtype]]$susie.pip[[j]], as.integer(dscout.susierss.list[[Rtype]]$meta[[j]]$true_coef!=0))
...
```{r echo=FALSE,fig.width=20, fig.height=15,fig.align='center'}
g = list()
idx = 0
for(Rtype in names(pip_cali)){
  idx = idx + 1
...
grid.arrange(g[[1]], g[[2]], g[[3]],g[[4]], g[[5]], g[[6]], g[[7]], g[[8]], g[[9]], g[[10]], g[[11]], g[[12]],nrow = 3)
```
...
saveRDS(roc, 'output/r_compare_add_z_lambda_SuSiErss_ROC.rds')
```

```{r}
roc = readRDS('output/r_compare_add_z_lambda_SuSiErss_ROC.rds')
chunks = 0
smooth = FALSE
