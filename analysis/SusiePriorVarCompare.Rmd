...
From this comparison, we can see the objective using `optim(Brent)` is higher than other methods.

Let's check the difference between objectives using Brent and BFGS:
```{r}
...
In the fitting procedure, both methods have 8 zero estimated prior variances at the 2nd iteration. On the 3rd iteration, zero prior variances become non-zero from `optim(Brent)`.
