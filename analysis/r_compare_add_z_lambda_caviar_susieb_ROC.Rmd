...
dscout.susiebhat = dscout.susiebhat[,-c(2, 6, 8, 13:18)]
colnames(dscout.susiebhat) = c('DSC', 'pve', 'n_signal', 'meta','N_in', 'ld_method','estimate_residual_variance', 'addz', 'L','pip', 'converged')

# only success cases
dscout.l1 = dscout.susiebhat %>% filter(converged == 1 & L == 1)
dscout.l2 = dscout.susiebhat %>% filter(converged == 1 & L == 2)
dscout.l5 = dscout.susiebhat %>% filter(converged == 1 & L == 5)
```
...
### L = 1
```{r}
dscout.susieb.est = dscout.l1 %>% filter(estimate_residual_variance==TRUE & addz == FALSE)
dscout.susieb.notest = dscout.l1 %>% filter(estimate_residual_variance==FALSE & addz == FALSE)

dscout.susieb.addz.est = dscout.l1 %>% filter(estimate_residual_variance==TRUE & addz == TRUE)
dscout.susieb.addz.notest = dscout.l1 %>% filter(estimate_residual_variance==FALSE & addz == TRUE)

dscout.susieb.list = list('in_sample.est' = dscout.susieb.est %>% filter(ld_method == 'in_sample'),
                          'in_sample.notest' = dscout.susieb.notest %>% filter(ld_method == 'in_sample'),
                          'in_sample.addz.est' = dscout.susieb.addz.est %>% filter(ld_method == 'in_sample'),
                          'in_sample.addz.notest' = dscout.susieb.addz.notest %>% filter(ld_method == 'in_sample'),
                          'out_sample.est' = dscout.susieb.est %>% filter(ld_method == 'out_sample'),
                          'out_sample.notest' = dscout.susieb.notest %>% filter(ld_method == 'out_sample'),
                          'out_sample.addz.est' = dscout.susieb.addz.est %>% filter(ld_method == 'out_sample'),
                          'out_sample.addz.notest' = dscout.susieb.addz.notest %>% filter(ld_method == 'out_sample'))
dat = list()
for(name in names(dscout.susieb.list)){
...
g = list()
idx = 0
name = names(pip_cali)[!grepl('in_sample.addz', names(pip_cali))]
for(Rtype in name){
  idx = idx + 1
...
saveRDS(roc, 'output/r_compare_add_z_lambda_caviar_SuSiEb_L1_ROC.rds')
```

```{r}
roc = readRDS('output/r_compare_add_z_lambda_caviar_SuSiEb_L1_ROC.rds')
chunks = 0
smooth = FALSE
...
### L = 2
```{r}
dscout.susieb.est = dscout.l2 %>% filter(estimate_residual_variance==TRUE & addz == FALSE)
dscout.susieb.notest = dscout.l2 %>% filter(estimate_residual_variance==FALSE & addz == FALSE)

dscout.susieb.addz.est = dscout.l2 %>% filter(estimate_residual_variance==TRUE & addz == TRUE)
dscout.susieb.addz.notest = dscout.l2 %>% filter(estimate_residual_variance==FALSE & addz == TRUE)
...
saveRDS(roc, 'output/r_compare_add_z_lambda_caviar_SuSiEb_L2_ROC.rds')
```

```{r}
roc = readRDS('output/r_compare_add_z_lambda_caviar_SuSiEb_L2_ROC.rds')
```
...
### L = 5
```{r}
dscout.susieb.est = dscout.l5 %>% filter(estimate_residual_variance==TRUE & addz == FALSE)
dscout.susieb.notest = dscout.l5 %>% filter(estimate_residual_variance==FALSE & addz == FALSE)

dscout.susieb.addz.est = dscout.l5 %>% filter(estimate_residual_variance==TRUE & addz == TRUE)
dscout.susieb.addz.notest = dscout.l5 %>% filter(estimate_residual_variance==FALSE & addz == TRUE)
...
saveRDS(roc, 'output/r_compare_add_z_lambda_caviar_SuSiEb_L5_ROC.rds')
```

```{r}
roc = readRDS('output/r_compare_add_z_lambda_caviar_SuSiEb_L5_ROC.rds')
```
