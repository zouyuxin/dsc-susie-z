...
There are 3 models from susie_bhat fail to converge. There are 18 cases with out-sample R failed (out of 608). The estimated residual variance becomes negative.
```{r}
converge.summary = aggregate(converged ~ ld_method + addz + estimate_residual_variance + L, dsc.susiebhat, sum)
converge.summary$Fail = 608 - converge.summary$converged
...
Using MAF threshold, there is 1 model from susie_bhat fail to converge. There is 1 model with out-sample R failed (out of 608). The estimated residual variance becomes negative.
```{r}
...
converge.summary$NotConverge = 608 - converge.summary$converged
