---
title: "Compare in-sample R vs out-sample R + z FINEMAP"
author: "Yuxin Zou"
date: "6/17/2019"
output: 
...
```{r}
dsc = readRDS('output/r_compare_maf_dscout_finemap_maf0_tibble.rds')
dsc = dsc[,-c(2,3)]
colnames(dsc) = c('DSC', 'pve', 'n_signal', 'meta', 'ld_method', 'L','add_z','pip')

dsc.maf = readRDS('output/r_compare_maf_dscout_finemap_maf0.05_tibble.rds')
dsc.maf = dsc.maf[,-c(2,3)]
colnames(dsc.maf) = c('DSC', 'pve', 'n_signal', 'meta', 'ld_method', 'L','add_z','pip')
```

## FINEMAP (max causal = 2)

```{r}
dsc.finemap.list = list('in_sample' = dsc %>% filter(L == '--n-causal-max 2' & ld_method == 'in_sample' & add_z == FALSE),
                        'out_sample' = dsc %>% filter(L == '--n-causal-max 2' & ld_method == 'out_sample' & add_z == FALSE),
                        'out_sample.addz' = dsc %>% filter(L == '--n-causal-max 2' & ld_method == 'out_sample' & add_z == TRUE),
                        'maf.in_sample' = dsc.maf %>% filter(L == '--n-causal-max 2' & ld_method == 'in_sample' & add_z == FALSE),
                        'maf.out_sample' = dsc.maf %>% filter(L == '--n-causal-max 2' & ld_method == 'out_sample' & add_z == FALSE),
                        'maf.out_sample.addz' = dsc.maf %>% filter(L == '--n-causal-max 2' & ld_method == 'out_sample' & add_z == TRUE))
```

```{r}
dat = list()
for(name in names(dsc.finemap.list)){
  dat[[name]] = matrix(NA, 0, 2)
}
for(Rtype in names(dat)){
  for(j in 1:nrow(dsc.finemap.list[[Rtype]])){
    datj = cbind(dsc.finemap.list[[Rtype]]$pip[[j]], as.integer(dsc.finemap.list[[Rtype]]$meta[[j]]$true_coef!=0))
...
saveRDS(roc, 'output/r_compare_mafall_FINEMAP_L2_ROC.rds')
```

```{r fig.width=6, fig.height=5}
roc = readRDS('output/r_compare_mafall_FINEMAP_L2_ROC.rds')
chunks = 0
smooth = FALSE
...
```{r}
dsc.finemap.list = list('in_sample' = dsc %>% filter(L == '--n-causal-max 5' & ld_method == 'in_sample' & add_z == FALSE),
                        'out_sample' = dsc %>% filter(L == '--n-causal-max 5' & ld_method == 'out_sample' & add_z == FALSE),
                        'out_sample.addz' = dsc %>% filter(L == '--n-causal-max 5' & ld_method == 'out_sample' & add_z == TRUE),
                        'maf.in_sample' = dsc.maf %>% filter(L == '--n-causal-max 5' & ld_method == 'in_sample' & add_z == FALSE),
                        'maf.out_sample' = dsc.maf %>% filter(L == '--n-causal-max 5' & ld_method == 'out_sample' & add_z == FALSE),
                        'maf.out_sample.addz' = dsc.maf %>% filter(L == '--n-causal-max 5' & ld_method == 'out_sample' & add_z == TRUE))
```
...
saveRDS(roc, 'output/r_compare_mafall_FINEMAP_L5_ROC.rds')
```

```{r fig.width=6, fig.height=5}
roc = readRDS('output/r_compare_mafall_FINEMAP_L5_ROC.rds')
labels = vector()
i = 1
name = names(roc)[!grepl('in_sample.addz', names(roc))]
for (method in name) {
