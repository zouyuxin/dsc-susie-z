...
dscout = dscout[,-c(2,6,10:18)]
colnames(dscout) = c('DSC', 'pve', 'n_signal', 'meta','N_in', 'method', 'ld_method','finemap.pip','add_z')
```
...
dscout.finemap.list = list('in_sample' = dscout.finemap[dscout.finemap$ld_method == 'in_sample',],
                           'out_sample' = dscout.finemap[dscout.finemap$ld_method == 'out_sample',],
                           'out_sample.addz' = dscout[dscout$method == 'finemap_add_z',])
```

```{r}
dat = list('in_sample'=matrix(NA, 0, 2), 'out_sample'=matrix(NA, 0, 2), 'out_sample.addz'=matrix(NA, 0, 2))
...
pip_cali = list('in_sample'=matrix(NA, nrow(bins), 3), 'out_sample'=matrix(NA, nrow(bins), 3), 'out_sample.addz'=matrix(NA, nrow(bins), 3))
...
saveRDS(roc, 'output/r_compare_add_z_lambda_FINEMAP_ROC.rds')
```

```{r fig.width=6, fig.height=5}
roc = readRDS('output/r_compare_add_z_lambda_FINEMAP_ROC.rds')
chunks = 0
smooth = FALSE
