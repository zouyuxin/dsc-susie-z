...
for(method in names(dat.susieb)){
  for(j in 1:nrow(dscout.equal.susieb.suc.list[[method]])){
    datj = cbind(dscout.equal.susieb.suc.list[[method]]$pip[[j]], dscout.equal.susieb.suc.list[[method]]$meta[[j]]$true_coef)
...
saveRDS(dat.susieb, 'output/r_compare_susieb_PIP_ROC.rds')
saveRDS(dat.susierss, 'output/r_compare_susierss_PIP_ROC.rds')
```

```{r}
dat = readRDS('output/r_compare_susieb_PIP_ROC.rds')
```
